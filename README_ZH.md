...
├── ai/                     # AI处理
│   ├── model_router.py     # 智能路由（简单→Flash-Lite，高质量→Flash）
│   ├── batch_processor.py  # 批量处理（每次5篇）+ 图片分析
...
| PDF导出失败 | 仅在Windows环境下可用（使用Windows字体） |
...
| AI | Google Gemini (Flash + Flash-Lite) | 免费层、智能路由 |
...
个人项目。All rights reserved.

---

*使用 Streamlit + Google Gemini AI 构建*
