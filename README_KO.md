...
| 탭 | 기능 |
|----|------|
| **📋 브리핑** | 오늘의 TOP 5 + 감성 온도계 (Plotly 차트 3종) |
| **📰 뉴스** | 전체 기사 + 필터 + 북마크/읽음 버튼 |
...
├── app.py                  # 메인 대시보드 (7탭)
├── config.py               # 설정
├── requirements.txt        # 의존성 (8개 패키지)
├── .env.example            # API 키 템플릿 (35개 플랫폼)
├── LICENSE                 # MIT 라이선스 (SoDam AI Studio)
├── .streamlit/config.toml  # 테마 + 포트 설정
├── crawler/                # 뉴스 수집
├── ai/                     # AI 처리 (라우팅, 배치, 채팅 등)
├── reader/                 # 원문 리더
...
| Phase 2-B | 음성 + 텔레그램봇 + 팩트체크 + 용어사전 + GitHub Actions | 예정 |
