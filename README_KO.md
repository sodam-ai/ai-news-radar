...
> **이게 뭐 하는 건가요?** 앱이 실행되는 데 필요한 도구(라이브러리)들을 자동으로 다운로드합니다.
...
├── ai/                     # AI 처리
│   ├── model_router.py     # 스마트 라우팅 (단순→Flash-Lite, 고품질→Flash)
│   ├── batch_processor.py  # 배치 처리 (5개씩) + 이미지 분석
...
| PDF 내보내기 실패 | Windows 환경에서만 동작 (Windows 폰트 사용) |
...
| AI | Google Gemini (Flash + Flash-Lite) | 무료 티어, 스마트 라우팅 |
...
개인 프로젝트입니다. All rights reserved.

---

*Streamlit + Google Gemini AI로 만들었습니다*
